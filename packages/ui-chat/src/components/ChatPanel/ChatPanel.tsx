...
export const ChatPanel = ({ spaceId, status, isLoading, setStatus, className }: ChatPanelProps) => {
  ...
  const startGoals = currentInformation?.startMode === constants.START_MODE_GOALS;
  const startQuestions = currentInformation?.startMode === constants.START_MODE_QUESTIONS;
  const hasGoals = (spaceGreeting as any)?.goals?.length > 0 && startGoals;
  const hasQuestions =
    (spaceGreeting as any)?.questions?.length > 0 &&
    messages.length === 0 &&
    (startQuestions || !hasGoals);
  ...
  const showInputGoalsButton = customConfig?.inputChat?.showGoalsButton ?? true;
  ...
  const handleView = async (view: string, thread = {}, preview = false) => {
    setCurrentInformation({
      thread,
      preview,
  ...
  const handleSendFeedback = async (props: {
    ...
    try {
      ...
      addMutation(async () => {
        ...
        await apiClient.fetcher(`/spaces/${spaceId}/thumb-signal/${getLastSignalId}/feedback`, {
          method: "POST",
          body: JSON.stringify({
            feedback,
    ...
    return (
      <div className="sharelyai-webcontroller-container-input">
        ...
        <ChatInput
          ...
          showGoalsButton={showInputGoalsButton}
          onGoalsClick={() => setStepActive(constants.GOALS_STEP)}
        />
      </div>
    );
  };
  ...
  return (
    <Wrapper
      className={classNames("sharely-chat-panel", className)}
    >
      <ScrollArea
        ...
        <div className="sharelyai-webcontroller-content-chat">
          ...
          {!isLoading &&
            ...
            messages.map((msg: any, index: number) => {
              ...
              if (msg?.newThread?.goalInstances?.length > 0) {
                return null;
              }

              const userName = msg?.user?.id === userData?.id ? "You" : msg?.user?.name;
              ...
              const thumbsData = thumbsSignals?.[thumbsSignals?.length - 1] || {};
