...
export const ListSearchItem = (props: ComponentProps) => {
  ...
  const customConfig =
    workspace?.spaceStyling?.customConfig?.views?.browse?.results?.listItem;
  ...
  const handleContainerClick = (event: React.MouseEvent<HTMLDivElement>) => {
    apiClient.spaces.sendEvent(currentInformation.spaceId, constants.SPACE_EVENTS.SPACE_EVENT_CLICKED_RESOURCE_IN_CATEGORY, {
        term: title,
        resultId: item.id,
        resultTitle: title,
        blobType,
        ...
        <div className="title">
          ...
          <div className="content">
            <div className="wrapper-title">
              ...
              {showPill && (
                <span className="pill">{!isChunk ? "File" : "Content"}</span>
              )}
            </div>
            <span className="description">
              ...
              {item?.score > 0 && <RelevantScore score={item.score} />}
              {page && (
                <>
                  <Divider type="dot" />
                  <span className="item">Page {page} </span>
                </>
              )}
            </span>
          </div>
