...
export const ChatHistory = (props: ChatHistoryProps) => {
  ...
  const regularGroups = (space as any)?.spaceGroupConversation as IGroupConversation[] | undefined;
  ...
  const handleGetChat = (group: IGroupConversation) => {
    ...
    const goalData = isGoal
      ? spaceGoals?.find((goal: any) => goal?.goalId === group?.goalId)
      : {};
  ...
  const isLoadingData = isAgentMode ? agentThreads.isLoading : spaceOptions?.isLoading;
  const shouldBeShowOptions = isAgentMode ? (groups?.length ?? 0) > 0 : (groups?.length ?? 0) > 1;
  const isPublicSpace =
    (space as any)?.status === constants.SPACE_STATUS_PUBLIC || spaceOptions?.isLoading;
                              ...
                              className={classNames(
                                "chat-history-period-history",
                                {
                                  "selected-chat-preview": isAgentMode
                                    ? chat?.id === currentInformation?.agentThreadId
                                    : chat?.id === currentInformation?.currentGroupId,
                                },
                              )}
