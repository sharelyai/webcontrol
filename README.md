...
```bash
pnpm --filter @sharelyai/services build      # one package
pnpm --filter @sharelyai/demo dev            # the demo app
pnpm --filter @sharelyai/ui-shared... build  # a package + everything that depends on it
```
...
@sharelyai/ui-browse         ← Browse feature: BrowsePanel, CategoryTree, ContentView (also depends on ui-search)
@sharelyai/ui-agent-chat     ← Agent chat: SSE-streamed responses, thinking steps, tool calls, citations
    ↓
@sharelyai/webcontrol        ← Shell (apps/webcontrol): composes all features into the WebControl component
    ↓
...
|---------|----------|-------------|
| `@sharelyai/services` | `packages/services` | Foundation — API client, React hooks, Zustand store, types, auth, i18n |
| `@sharelyai/ui-shared` | `packages/ui-shared` | Shared UI primitives — theme, base components, icons |
| `@sharelyai/ui-chat` | `packages/ui-chat` | Standard chat panel with messages, goals, and workflow progress |
| `@sharelyai/ui-search` | `packages/ui-search` | Search panel with results and tag filtering |
| `@sharelyai/ui-browse` | `packages/ui-browse` | Browse panel with category tree and content view (depends on ui-search) |
| `@sharelyai/ui-agent-chat` | `packages/ui-agent-chat` | Agent chat — SSE-streamed responses with thinking indicators, tool-call cards, and citations |
| `@sharelyai/webcontrol` | `apps/webcontrol` | Shell that composes all feature packages into the `WebControl` component + embed bundle |
...
- **Styling** — styled-components v6 with CSS variables for theming. Each feature
  ...
  `@sharelyai/ui-shared`; pass a partial `theme` to override tokens.
- **Build** — Library packages use **tsup** (CJS + ESM dual output). Apps and the
  ...
  output.
- **Agent mode** — The agent chat feature (`@sharelyai/ui-agent-chat`) renders
...
`@sharelyai/webcontrol`). It mounts into the element with id
...
|-----|---------|-------|----------------|
| [services.md](./examples/services.md) | `@sharelyai/services` | Data layer only — API client, hooks, store, no UI | `/headless-demo` |
| [ui-shared.md](./examples/ui-shared.md) | `@sharelyai/ui-shared` | Theme system + base components + icons, no data | `/ui-shared` |
| [ui-chat.md](./examples/ui-chat.md) | `@sharelyai/ui-chat` | `ChatPanel` | `/chat-only` |
| [ui-search.md](./examples/ui-search.md) | `@sharelyai/ui-search` | `SearchPanel` | `/search-only` |
| [ui-browse.md](./examples/ui-browse.md) | `@sharelyai/ui-browse` | `BrowsePanel` (depends on ui-search) | `/browse-only` |
| [ui-agent-chat.md](./examples/ui-agent-chat.md) | `@sharelyai/ui-agent-chat` | `AgentChatPanel` — SSE streaming, thinking, tool calls, citations | `/agent-chat-only` |
...
- `/headless-demo` — headless hooks (build your own UI on `@sharelyai/services`)
...
Vercel target (`vercel.json` builds `@sharelyai/webcontrol` and its dependencies).
